    ...
    public function data(Request $request)
    {
        ...
        if (Auth::check()) {
            ...
            if (isset($bo)) {
                ...
                $user = User::find($request->user()->id);
                $bt = product1::where("id", $request->productid)->first();
//return $bt;
                ...
                curl_setopt_array($curl, array(
                    ...
                    CURLOPT_POSTFIELDS => array('service' => 'data', 'coded' => $bt->networkcode, 'phone' => $request->number, 'amount' => $request->amount, 'reseller_price' => $request->amount),
...
//                        return $response;
                if ($success == 1) {
                    ...
                    $name = $bt->plan;
                    $am = "$request->name  Purchase Was Successful To";
                    $ph = $request->number;
